...
try:
    ...
    logger.warning("sentence-transformers not available. Embeddings will be skipped.")
...
class KnowledgeProcessor:
    ...
    def generate_embeddings_for_concepts(self, concepts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for concept in concepts:
            text = (concept.get("description") or "") + " " + (concept.get("name") or "")
            text = text.strip()
    ...
    def semantic_deduplicate_concepts(self, concepts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for concept in concepts:
            ...
            for kept_embedding in deduped_embeddings:
                ...
                if similarity >= self.similarity_threshold:
                    is_duplicate = True
                    logger.debug(f"Duplicate found: {concept.get('name')} (similarity: {similarity:.3f})")
                    break
    ...
    def finalize_embeddings(self, concepts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def deduplicate_relations(self, relations: List[Dict[str, Any]], valid_concept_ids: set) -> List[Dict[str, Any]]:
        """
    ...
    def process(self, extraction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        extraction_result["meta"]["concepts_deduped"] = original_concept_count - len(concepts)
        extraction_result["meta"]["relations_deduped"] = original_relation_count - len(relations)
        
        logger.info(f"Processed: {len(concepts)} concepts, {len(relations)} relations "
